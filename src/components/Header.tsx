          ...
          {isAuthenticated ? (
            <div className="flex items-center gap-4 ml-4 pl-4 border-l border-white/20">
               ...
               <div className="flex flex-col items-end">
                  <span className={cn("text-[10px] font-bold uppercase leading-none", isAdmin ? "text-pl-green" : "text-gray-400")}>
      ...
      {isOpen && (
        <div className="md:hidden bg-pl-purple border-t border-pl-pink py-6 shadow-2xl animate-in slide-in-from-top-2">
          <nav className="flex flex-col gap-6 px-6 font-heading text-xl font-bold uppercase tracking-widest">
            ...
            {isAuthenticated ? (
                <button onClick={() => { logout(); setIsOpen(false); }} className="text-left py-2 text-red-400 font-bold border-l-4 border-transparent pl-4 uppercase cursor-pointer">
                    Logout ({isAdmin ? 'Admin' : 'Member'})
                </button>
            ) : (
                <Link to="/login" onClick={() => setIsOpen(false)} className="py-2 text-pl-green font-bold border-l-4 border-transparent pl-4 uppercase cursor-pointer">Login</Link>
            )}
          </nav>
        </div>
      )}
