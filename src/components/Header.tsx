...
export default function Header() {
  ...
  const { user, logout, isAdmin, isWhitelisted } = useAuth(); // Thêm isWhitelisted
  ...
  const isAuthenticated = !!user && isWhitelisted;
    ...
    <header className="bg-pl-purple text-white sticky top-0 z-50 shadow-md border-b-4 border-pl-green font-sans">
      <div className="container mx-auto px-4 h-20 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-3 group cursor-pointer">
          ...
          <span className="font-heading text-2xl font-bold uppercase tracking-tight group-hover:text-pl-green transition-colors">
            {teamName}
          </span>
        </Link>
        ...
        <nav className="hidden md:flex gap-8 font-heading text-sm font-bold uppercase items-center tracking-widest">
            ...
            <Link
              ...
              className={cn(
                ...
                location.pathname === item.path &&
                  "text-pl-green border-pl-green scale-105",
              )}
            >
              {item.label}
            </Link>
          ...
          {isAuthenticated ? (
            <div className="flex items-center gap-4 ml-4 pl-4 border-l border-white/20">
              ...
              <button
                onClick={() => logout()}
                className="bg-white/10 hover:bg-red-500 p-2 rounded-xl transition-all cursor-pointer shadow-inner"
                title="Logout"
              >
                <svg
        ...
        <div className="md:hidden flex items-center gap-3">
          ...
          <button
            className="text-white hover:text-pl-green transition-colors cursor-pointer"
          ...
          <nav className="flex flex-col gap-6 px-6 font-heading text-xl font-bold uppercase tracking-widest">
            {navItems.map((item) => (
              <Link
                ...
                to={item.path}
                className={cn(
                  "hover:text-pl-green block py-2 border-l-4 border-transparent pl-4 transition-all cursor-pointer",
                  location.pathname === item.path &&
                    "text-pl-green border-pl-green bg-white/5",
                )}
