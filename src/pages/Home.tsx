  ...
  const fetchData = useCallback(async () => {
    ...
    if (allPlayers && allStats) {
      ...
      const topScorer = [...playersWithStats].sort(
        (a, b) => b.calculatedStats.goals - a.calculatedStats.goals,
      )[0];
      let topAssister = [...playersWithStats]
        .filter((p) => p.id !== topScorer?.id)
        .sort(
          (a, b) => b.calculatedStats.assists - a.calculatedStats.assists,
        )[0];
      if (!topAssister)
        topAssister = [...playersWithStats].filter(
          (p) => p.id !== topScorer?.id,
        )[0];
      let ironMan = [...playersWithStats]
        .filter((p) => p.id !== topScorer?.id && p.id !== topAssister?.id)
        .sort(
          (a, b) => b.calculatedStats.matches - a.calculatedStats.matches,
        )[0];
      if (!ironMan)
        ironMan = [...playersWithStats].filter(
          (p) => p.id !== topScorer?.id && p.id !== topAssister?.id,
        )[0];
      let mvp = [...playersWithStats]
        ...
        .sort(
          (a, b) =>
            b.calculatedStats.contributionPerMatch -
            a.calculatedStats.contributionPerMatch,
        )[0];
      if (!mvp)
        mvp = [...playersWithStats].filter(
          (p) =>
            p.id !== topScorer?.id &&
            p.id !== topAssister?.id &&
            p.id !== ironMan?.id,
      ...
      setFeaturedPlayers(
        [topScorer, topAssister, ironMan, mvp].filter(Boolean),
      );
    }
    ...
    <div className="overflow-x-hidden">
      ...
      <section className="relative bg-pl-purple text-white overflow-hidden min-h-[500px] md:min-h-[600px] flex items-center">
        <div
          ...
          style={{
            backgroundImage: `url(${settings.banner_url || "https://images.unsplash.com/photo-1522778119026-d647f0565c6a?auto=format&fit=crop&q=80&w=2000"})`,
          }}
        ></div>
        ...
        <div className="container mx-auto px-4 relative z-10 py-12 md:py-20 flex flex-col md:flex-row items-center">
          <div className="md:w-2/3 text-left w-full">
            ...
            <h1 className="text-4xl md:text-8xl font-heading font-bold uppercase leading-tight md:leading-[0.9] mb-6 drop-shadow-xl tracking-tight">
              ...
              <br />
              <span className="text-pl-pink">
                {settings.team_name.split(" ").slice(1).join(" ")}
              </span>
            </h1>
              ...
              >
                ...
                <div className="flex justify-between items-center mb-6">
                  <div className="text-center w-1/3 group-hover:scale-110 transition-transform">
                    ...
                    <span className="font-heading text-lg leading-tight block uppercase tracking-tighter">
                      RealFake
                    </span>
                  </div>
                  <div className="text-2xl md:text-4xl font-heading font-bold text-center w-1/3 opacity-40 italic">
                    VS
                  </div>
                  <div className="text-center w-1/3">
                    ...
                    <span className="font-heading text-sm md:text-lg leading-tight block uppercase tracking-tighter truncate">
                      {nextMatch.opponent}
                    </span>
                  </div>
                </div>
                <div className="text-center bg-pl-purple/40 p-3 rounded-xl border border-white/5 shadow-inner text-white">
                  <div className="text-2xl md:text-3xl font-bold font-heading tabular-nums text-pl-green">
                    {nextMatch.time.substring(0, 5)}
                  </div>
        ...
        <div className="container mx-auto px-4 grid md:grid-cols-2 gap-10 md:gap-16 items-center">
          ...
          <div className="relative w-full">
            <img
              ...
              alt="Team Formation"
              className="rounded-2xl md:rounded-[3rem] shadow-xl w-full object-cover h-[250px] md:h-[400px]"
            />
          </div>
        </div>
      </section>
              ...
              <div className="md:w-1/3 w-full max-w-[250px] md:max-w-none">
                <div className="relative mx-auto md:mx-0">
                  ...
                  <img
                    ...
                    alt={manager.name}
                    className="relative z-10 w-full drop-shadow-2xl rounded-b-full border-b-4 md:border-b-8 border-pl-green"
                  />
                </div>
              </div>
              <div className="md:w-2/3 text-left w-full">
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center md:flex-col md:text-center gap-4 hover:border-pl-purple transition-all group">
              <div className="w-12 h-12 md:w-16 md:h-16 bg-pl-purple/5 rounded-xl flex items-center justify-center group-hover:bg-pl-purple group-hover:text-white transition-all text-pl-purple shrink-0">
                <Clock size={24} />
              </div>
              <div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center md:flex-col md:text-center gap-4 hover:border-pl-green transition-all group">
              <div className="w-12 h-12 md:w-16 md:h-16 bg-pl-green/10 rounded-xl flex items-center justify-center group-hover:bg-pl-green group-hover:text-pl-purple transition-all text-pl-green shrink-0">
                <MapPin size={24} />
              </div>
              <div>
                <h3 className="text-sm md:text-xl font-heading font-bold uppercase md:mb-2 leading-none">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center md:flex-col md:text-center gap-4 hover:border-pl-pink transition-all group">
              <div className="w-12 h-12 md:w-16 md:h-16 bg-pl-pink/10 rounded-xl flex items-center justify-center group-hover:bg-pl-pink group-hover:text-white transition-all text-pl-pink shrink-0">
                <Phone size={24} />
              </div>
              <div>
                <h3 className="text-sm md:text-xl font-heading font-bold uppercase md:mb-2 leading-none">
                ...
                <a
                  ...
                  className="text-sm md:text-lg font-bold hover:underline tabular-nums leading-none block"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {latestMatches.map((match) => (
                <Link
                  ...
                  className="block transform hover:scale-[1.02] transition-all h-full cursor-pointer"
                >
