...
export default function Admin() {
  ...
  if (authLoading) return <div className="text-center py-20 font-heading text-pl-purple uppercase animate-pulse">Đang xác minh quyền admin...</div>;
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-gray-50 px-4 text-pl-purple">
        <div className="bg-white p-10 rounded-3xl shadow-2xl w-full max-w-sm text-center border-t-8 border-red-500">
          <h2 className="text-3xl font-heading font-bold uppercase mb-4">Truy cập bị từ chối</h2>
          <p className="mb-10 text-gray-500 text-sm">Chỉ dành cho Quản trị viên.</p>
          <Link to="/" className="block bg-pl-purple text-white w-full py-4 rounded-2xl font-bold hover:bg-pl-pink transition-all uppercase text-xs cursor-pointer shadow-lg text-center">Về trang chủ</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-10 min-h-screen text-pl-purple font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 border-b border-gray-100 pb-6 gap-4">
        <div>
            <h1 className="text-4xl md:text-5xl font-heading font-bold uppercase leading-none">Bảng điều khiển</h1>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-2">RealFake FC • Quản trị</p>
        </div>
        <div className="bg-pl-gray/20 p-3 rounded-2xl border border-white flex items-center gap-3">
            <div className="text-right">
                <div className="text-[9px] font-bold text-pl-pink uppercase leading-none">Siêu người dùng</div>
      ...
      <div className="flex gap-3 mb-10 overflow-x-auto pb-2 scrollbar-hide snap-x">
        {['players', 'matches', 'settings'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab as any)} className={`px-6 md:px-8 py-3 rounded-2xl font-bold transition-all uppercase text-[10px] tracking-widest cursor-pointer whitespace-nowrap shadow-sm active:scale-95 snap-start ${activeTab === tab ? 'bg-pl-purple text-white' : 'bg-white text-gray-400 border border-gray-100 hover:border-pl-purple hover:text-pl-purple'}`}>
                {tab === 'players' ? 'Quản lý đội hình' : tab === 'matches' ? 'Quản lý lịch đấu' : 'Cài đặt đội'}
            </button>
        ))}
      </div>

      {loading && <div className="fixed top-4 right-4 bg-pl-green text-pl-purple font-bold px-4 py-2 rounded shadow animate-pulse z-50 text-[10px]">ĐANG ĐỒNG BỘ...</div>}
      ...
      {deleteTarget && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4 text-center">
              <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm border-t-8 border-red-500 p-8 animate-in zoom-in-95 duration-200">
                  ...
                  <h3 className="text-2xl font-heading font-bold text-pl-purple uppercase mb-2">Xóa vĩnh viễn?</h3>
                  <p className="text-gray-500 text-sm mb-8 font-medium">Xóa vĩnh viễn mục này...<br/>Dữ liệu sẽ mất mãi mãi.</p>
                  <div className="flex gap-3">
                      <button onClick={() => setDeleteTarget(null)} className="flex-1 py-3.5 font-bold text-gray-500 bg-gray-100 rounded-2xl cursor-pointer hover:bg-gray-200 transition-all uppercase text-[10px] tracking-widest">Hủy</button>
                      <button onClick={confirmDelete} className="flex-1 py-3.5 font-bold text-white bg-red-500 rounded-2xl cursor-pointer hover:bg-red-600 transition-all shadow-lg uppercase text-[10px] tracking-widest active:scale-95">Xóa</button>
                  </div>
              </div>
          </div>
      )}
